...
The existing-mask and threshold workflows use no learned weights. When
substituting LungMask, TotalSegmentator, nnU-Net or MONAI in 0.6.0, each node's
administrator must first provision a complete, digest-verified model bundle.
Inspect `ds.imaging.models(conns)` for `provider`, `task`, `ready` and
`manifest_sha256`; analysts supply names, never model paths. A missing or
changed bundle fails closed, with no model download during analysis. The
historical demonstration below has not been rerun for this release.
