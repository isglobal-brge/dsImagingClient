...
- Added
  ...
  and its exact cleanup helper so complete derived feature assets can be
  handed to dsFlower as opaque, patient-bound session capabilities
  without exposing a data frame in the workspace. Multi-column
  selections are encoded as one literal transport argument, so they
  remain usable with the server’s strict expression boundary.
